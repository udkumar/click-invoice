...
ActiveAdmin.register Invoice do
  ...
  index do
    ...
    column :total do |invoice|
      number_to_currency invoice.total, locale: :ru
    end
    ...
    panel "Products" do
      table_for invoice.items do |t|
        ...
        tr do
          ...
          td "#{number_with_delimiter(invoice.discount, locale: :ru)}%"
        end

        tr do
          2.times { td "" }
          td "Sub-total:", :style => "text-align:right; font-weight: bold;"
          td "#{number_to_currency(invoice.subtotal, locale: :ru)}"
        end

        tr do
          2.times { td "" }
          td "Taxes:", :style => "text-align:right; font-weight: bold;"
          td "#{number_to_currency(invoice.taxes, locale: :ru)} (#{number_with_delimiter(invoice.tax)}%)"
        end

        tr do
          2.times { td "" }
          td "Total:", :style => "text-align:right; font-weight: bold;"
          td "#{number_to_currency(invoice.total, locale: :ru)}", :style => "font-weight: bold;"
        end
      end
    end
  ...
  sidebar "Total", :only => :show do
    h1 number_to_currency(invoice.total, locale: :ru), :style => "text-align: center; margin-top: 20px"
  end
  ...
  form do |f|
    ...
    f.inputs "Other Fields" do
      f.input :terms, :input_html => { :rows => 4, :value => "You can order only 2 item with same category for single membership. If we found you are selling item from here, your membership will be cancelled."}, :label => "Terms & Conditions"
      f.input :notes, :input_html => { :rows => 4, :value => "If you are cancelled your order, you have to paid arranging cost. So please onder only item which you have decided strognly." }
    end
